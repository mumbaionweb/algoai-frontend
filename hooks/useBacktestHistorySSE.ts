  ...
  useEffect(() => {
    ...
    eventSource.onerror = () => {
      ...
      if (eventSource.readyState === EventSource.CLOSED && !hasReceivedConnectionRef.current) {
        console.error('[SSE_HISTORY] ❌ Connection closed immediately - likely 401 Unauthorized');
        setError('Authentication failed. Please refresh the page.');
        setLoading(false);
        isIntentionallyClosedRef.current = true;
        return;
      ...
      if (eventSource.readyState === EventSource.CLOSED) {
        if (!isIntentionallyClosedRef.current) {
          ...
          console.log('[SSE_HISTORY] 🔌 Connection closed (will auto-reconnect)');
        }
      } else if (eventSource.readyState === EventSource.CONNECTING) {
        console.log('[SSE_HISTORY] 🔄 Reconnecting...');
      } else {
        // Only log actual errors, not normal closures
        console.error('[SSE_HISTORY] ❌ Connection error:', {
